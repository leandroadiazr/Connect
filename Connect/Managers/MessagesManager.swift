...
class MessagesManager {
    ...
    //MARK:- OBSERVE RECIPIENT PROFILE
    ...
    //MARK:- GET ALL USERS IN CHAT CONTROLLERS
    ...
    func observeSingleSenderRecipientConversation(for recipient: String, completion: @escaping (Result<[Messages], ErrorMessages>)-> Void) {
        ...
        self.database.child("userMessages").child(currentUserID).child(recipient).observe( .childAdded) { keySnap in
    ...
    func observeDiscussions(completion: @escaping (Result<[Messages], ErrorMessages>)-> Void) {
        ...
        self.database.child("userMessages").child(currentUserID).observe( .childAdded) { keySnap in
            ...
            self.database.child("userMessages").child(currentUserID).child(childId).observe( .childAdded) { childSnap in
                ...
                self.database.child("messages").child(nodeID).observeSingleEvent(of: .value) { snapshot in
                    guard let data = snapshot.value else {
                        completion(.failure(.failedToFechFromDatabase))
                        print("unable to fetch message please change to error messages")
                        return }
